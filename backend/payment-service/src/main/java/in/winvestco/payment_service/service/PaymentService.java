package com.trading.payment_service.service;

import com.trading.common.enums.PaymentMethod;
import com.trading.common.enums.PaymentProvider;
import com.trading.common.enums.PaymentStatus;
import com.trading.payment_service.dto.*;
import com.trading.payment_service.exception.PaymentNotFoundException;
import com.trading.payment_service.exception.PaymentVerificationException;
import com.trading.payment_service.mapper.PaymentMapper;
import com.trading.payment_service.messaging.PaymentEventPublisher;
import com.trading.payment_service.model.Payment;
import com.trading.payment_service.repository.PaymentRepository;
...
public class PaymentService {
    ...
    public PaymentResponse verifyPayment(Long userId, VerifyPaymentRequest request) {
        return Observation.createNotStarted("payment.verify", observationRegistry)
                .contextualName("verify-payment-" + request.getRazorpayOrderId())
                .observe(() -> {
                    ...
                    if (!isValid) {
                        ...
                        meterRegistry.counter("payment.failed.count", "reason", "SIGNATURE_VERIFICATION_FAILED")
                                .increment();
                    ...
                    meterRegistry.counter("payment.success.count", "method", payment.getPaymentMethod().name())
                            .increment();
    ...
    private void handlePaymentSuccess(Payment payment, RazorpayWebhookPayload.PaymentEntity entity) {
        ...
        meterRegistry.counter("payment.success.count", "method", method.name()).increment();
    ...
    private void handlePaymentFailed(Payment payment, RazorpayWebhookPayload.PaymentEntity entity) {
        ...
        meterRegistry.counter("payment.failed.count", "reason",
                entity.getErrorCode() != null ? entity.getErrorCode() : "UNKNOWN").increment();
